...
class License {
     ...
     *
     * @see https://regex101.com/r/AJS0cF/2
     */
    const VALIDATE_NEW_HOSTNAME_SKIP_BY_REGEXP = '/^(?:\\w+-\\d+-\\d+-\\d+-\\d+\\.[^\\.]+\\.amazonaws\\.com)/m';
    /**
     * Plugin slug.
     *
     * @var string
        ...
        if ($oldValue !== null) {
            $activation->deactivate(
                ...
                \sprintf(
                    // translators:
                    __(
                        'The plugin has a new update server. Therefore, you need to reactivate your license (%s) to continue receiving updates.',
                        RPM_WP_CLIENT_TD
                    ),
                    $oldValue
                ) .
                    (is_multisite()
                        ? ' ' .
                            __(
                                'You are using a WordPress multisite. According to the plugin\'s licence agreement, you need one license per website. If you have only used one licence for all websites in your WordPress multisite, this was only possible because it was not technically prevented. We ask for your understanding if this causes you any inconvenience!',
                                RPM_WP_CLIENT_TD
                            )
                        : '')
            );
    ...
    public function syncWithRemote() {
        ...
        if (empty($code)) {
            return new \WP_Error(
                \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\license\License::ERROR_CODE_NOT_ACTIVATED,
    ...
    public function update_option_siteurl() {
        $currentHostname = \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\Utils::getCurrentHostname();
    ...
    public function validateNewHostName() {
        ...
        if (
            ...
            !$dynamic &&
            !\in_array($currentHostname, self::VALIDATE_NEW_HOSTNAME_SKIP, \true) &&
            !\preg_match(self::VALIDATE_NEW_HOSTNAME_SKIP_BY_REGEXP, $currentHostname)
        ) {
            ...
            if ($currentHostname !== $persistedHostname) {
                ...
                update_option(
                    \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\license\License::OPTION_NAME_UUID_PREFIX .
                        $this->getSlug(),
                    ''
                );
    ...
    public function validateRemoteResponse($response) {
        ...
        if (
            ...
            $response->get_error_code() ===
                \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\client\ClientUtils::ERROR_CODE_REMOTE
        ) {
    ...
    public function activateProgrammatically($force = \false) {
        ...
        if ($prog === \false) {
            return new \WP_Error(
                'rpm_wpc_programmatic_activation_not_found',
                __('No programmatic activation found for this plugin.')
            );
        }
        ...
        if (is_wp_error($result)) {
            update_option(self::OPTION_NAME_HINT_PREFIX . $this->getSlug(), [
                'validateStatus' => 'error',
                'hasFeedback' => \true,
                'help' => \trim(\join(' ', $result->get_error_messages()))
            ]);
        }
        return $result;
    }
    ...
    public function receivedRemoteLicenseActivation($licenseActivation) {
        ...
        $result = update_option(
            \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\license\License::OPTION_NAME_LICENSE_ACTIVATION_PREFIX .
                $this->getSlug(),
            $licenseActivation
        );
        $this->restore();
    ...
    public function getInitiator() {
        return \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\Core::getInstance()->getInitiator(
            $this->getSlug()
        );
    }
    /**
    ...
    public function getAsArray() {
        ...
        $this->switch();
        $host = \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\Utils::getCurrentHostName();
        $this->restore();
        return [
            'blog' => $this->getBlogId(),
            'host' => $host,
    ...
    public function getProgrammaticActivation() {
        ...
        $result = apply_filters(
            'DevOwl/RealProductManager/License/Programmatic/' . $this->getSlug(),
            \false,
            $this->getBlogId(),
            \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\Utils::getCurrentHostname()
        );
