...
class Emage_Hover_Effects {
    ...
    public function insert_plugin_row_meta($links, $file) {
        if (plugin_basename(__FILE__) == $file) {
            // docs
            $links[] = sprintf('<a href="https://docs.blocksera.com/emage-hover-effects-for-elementor?utm_source=wp&utm_medium=admin&utm_term=emage" target="_blank">' . __('Docs', 'ehe-lang') . '</a>');
        }
    ...
    public function check_license() {
        ...
        $license_key = sanitize_key($_POST['code']);
        ...
        switch ($license_action) {
            ...
            case 'activate':
                ...
                if ($update->license_error == 'limit_exceeded') {
                    $response['message'] = __('You have already used this code on another site.<br><br>Please deactivate license from the site or buy another license', 'ehe-lang');
                } else if ($update->license == 'regular' || $update->license == 'extended') {
                    $config['license'] = $update->license;
            ...
            case 'deactivate':
                ...
                if ($update->license_error == 'site_removed') {

                    $response['message'] = __('Your license has been removed from this site. It can now be used again.<br><br>Please update and refresh this page', 'ehe-lang');
                    $config['license_key'] = '';
                    $config['license'] = $update->license;

                } else {
                    $response['message'] = __('Your purchase code is not valid. Please try again.', 'ehe-lang');
                }
        ...
        update_option('ehe_config', $config);
