...
class ITSEC_Hide_Backend {
	...
	public function notification_strings() {
		return array(
			'label'       => __( 'Hide Backend – New Login URL', 'it-l10n-ithemes-security-pro' ),
			'description' => sprintf(
				__( '%1$sHide Backend%2$s will notify the chosen recipients whenever the login URL is changed.', 'it-l10n-ithemes-security-pro' ),
				ITSEC_Core::get_link_for_settings_route( ITSEC_Core::get_settings_module_route( 'hide-backend' ) ),
				'</a>'
			),
			'subject'     => __( 'WordPress Login Address Changed', 'it-l10n-ithemes-security-pro' ),
			'message'     => __( 'The login address for {{ $site_title }} has changed. The new login address is {{ $login_url }}. You will be unable to use the old login address.', 'it-l10n-ithemes-security-pro' ),
			'tags'        => array(
				'login_url'  => __( 'The new login link.', 'it-l10n-ithemes-security-pro' ),
